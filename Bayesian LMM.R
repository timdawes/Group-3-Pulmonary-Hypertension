...
# Links to Figure 6 / Figure 6 code (Association of PDE5 and ERA with echocardiography, haemodynamic and functional markers)
...
# List of markers of interest (ns) and their labels( ns.labels) and a count of how many their are (n)
      ...
      ns.labels<- c("Six-minute walk distance (m)","Functional Class","emPHasis-10","Tricuspid annulus planar","Heart rate (bpm)","Cardiac output (L/min)","Pulmonary vascular","Mean pulmonary artery","B-type natriuretic peptide (ng/L)","Kco (/s)","Arterial saturation (%)","Venous saturation (%)")
      ns.labels2<- c("","","","systolic excursion (cm)","","","resistance (WU)","pressure (mmHg)","","","","")
      n<- length(ns)

# Construct formulae to analyses these markers
      fmla.mm<- paste(ns, c("~ "), c("DoseEqPDE5 +  DoseEqERA + RxTime + DoseEqPDE5*RxTime + DoseEqERA*RxTime + DoseEqPDE5*SubgroupOne + DoseEqPDE5*SubgroupTwo + DoseEqERA*SubgroupOne + DoseEqERA*SubgroupTwo + (1 | IDn)"))
      fmla.mm.MCMC.fixed<- paste(ns, c("~ "), c("DoseEqPDE5 +  DoseEqERA + RxTime + DoseEqPDE5*RxTime + DoseEqERA*RxTime + DoseEqPDE5*SubgroupOne + DoseEqPDE5*SubgroupTwo + DoseEqERA*SubgroupOne + DoseEqERA*SubgroupTwo"))

# Make matrices to store the analysis results
      func.cols<- c("Intercept","DoseEqPDE5","DoseEqERA","RxTime","SubgroupOne","SubgroupTwo",
                    "DoseEqPDE5:RxTime","DoseEqERA:RxTime","DoseEqPDE5:SubgroupOne","DoseEqPDE5:SubgroupTwo",
                    "DoseEqERA:SubgroupOne","DoseEqERA:SubgroupTwo")
...
# Loop through each formula analysing it
      for (i in 1:length(fmla.mm))
      {
        ...
        for (x in 1:imputations) 
        {
          ...
          cols.mm<- match(c("DoseEqPDE5","DoseEqERA","AgeAtDiag", "tstart","tstop","AgeAtTest","IDn", ns), colnames(results.olps.i.amelia$imputations[[1]]))
          ...
          # Format the data
          # Convert diagnosis column into binary '1/0' columns for subgroups 3.1, 3.2 and other
          diagnosis.cat<- results.olps.i.amelia$imputations[[x]]$Diagnosis
          levels(diagnosis.cat)[levels(diagnosis.cat) %in% c("3.3","3.4","3.5","3.7")]<- "Other"
          ...
          colnames(diagnosis_cat)<- c("SubgroupOne","SubgroupTwo","SubgroupOther")
          data.sc.func<- cbind(data.sc.func, diagnosis_cat[,-3])
          ...
          data.sc.func2<- NULL
          for (k in unique(data.sc.func$IDn))
          {
            ages<- data.sc.func[which(data.sc.func$IDn==k),]$AgeAtTest
            sc.data.per.pt<- data.sc.func[which(data.sc.func$IDn==k)[order(ages)],]
            first.Rx.date.PDE5<- sc.data.per.pt[which(sc.data.per.pt$DoseEqPDE5!=0)[1],]$AgeAtTest
            first.Rx.date.ERA<- sc.data.per.pt[which(sc.data.per.pt$DoseEqERA!=0)[1],]$AgeAtTest
            first.Rx.date.overall<- min(na.omit(c(first.Rx.date.PDE5, first.Rx.date.ERA)))
            if (first.Rx.date.overall==Inf) {first.Rx.date.overall<- max(sc.data.per.pt$AgeAtTest)}
            sc.data.per.pt$RxTime<- sc.data.per.pt$AgeAtTest - first.Rx.date.overall
          ...
          for (j in 1:length(cols.sc))
          {
            from<- 2*(j-1) + 1
            to<- from + 1
            scaling.data[i,from:to]<- c(mean(data.sc.func[,cols.sc[j]]), sd(data.sc.func[,cols.sc[j]]))
          ...
          # Fit the model        
          #fit_me<- blmer(as.formula(fmla.mm[i]), data = data.sc.func2, cov.prior = gamma)
      ...
      matrix.mean<- matrix.lowerCI<- matrix.upperCI<- matrix.sd[[model]]<- coef.df.mm<- list()
      imputations<- 100
