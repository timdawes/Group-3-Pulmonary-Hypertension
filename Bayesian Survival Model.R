...
# Fitting of Bayesian Accelerated Failure Time model 

        # Define MCMC parameters
            imputations<- 100
            res.bayes<- list()
            nburn=1000; nsave=4000; nskip=100 # Note larger nburn, nsave and nskip should be used in practice.
      ...
      # Setting priors
          ...
          # Fit a gamma distribution to the FU times and extract the two coefficients which define it
                ...
                scaling.data<- matrix(0, nrow=imputations, ncol=16, 
                                      dimnames=list(paste("I",1:imputations,sep=""), 
                                                    c("DoseEqPDE5_Mean","DoseEqPDE5_sd",
                                                      "DoseEqERA_Mean","DoseEqERA_sd",
      ...
      # Choose a subgroup
            newbit<- c("~ DoseEqPDE5 + DoseEqERA + AgeAtDiag + Gender + TAPSE + FVC + PVR + BNP + SubgroupOne + SubgroupTwo",
                       "~ DoseEqPDE5 + DoseEqERA + DoseEqPDE5*PVR + DoseEqERA*PVR")
                       
                       
            repeat {
              round<- round + 1
              if (round>length(betas.sa)) {break}
              
                    for (model in 1:2)
                          {
                          res.bayes[[model]]<- list()
                          ...
                          for (i in 1:imputations)
                                  {
                                      ...
                                      # Pick out the repeat observations of the patients in the subset of patients of interest
                                            ...
                                            # Screen text to confirm which cohort of patients you're currently working on
                                                  t<- table(results.olps.i.amelia$imputations[[1]]$Diagnosis[which.rows])
                                                  cat("\n Diagnoses in cohort = ", names(t[which(t!=0)]), "\n \n")
                                            # Select out the subset of patients
                                                  which.cols<- match(c("DoseEqPDE5","DoseEqERA","AgeAtDiag", "TAPSE","Gender","FVC","mPAP","tstop","tstart","futime","Outcome","IDn","PVR","BNP"), colnames(results.olps.i.amelia$imputations[[i]]))
                                      ...
                                      # Convert diagnosis column into binary '1/0' columns for subgroups 3.1, 3.2 and other
                                                  diagnosis.cat<- results.olps.i.amelia$imputations[[i]]$Diagnosis
                                                  ...
                                                  data.sc<- cbind(data.sc, diagnosis_cat[,-3])
                                                  these.are.factors<- which(sapply(apply(data.sc,2,table),length)<=2)
                                                  for (k in these.are.factors) {data.sc[,k]<- as.factor(data.sc[,k])}            
                                                  
                                      # Scale and centre the data in the columns of interest
                                            # Pick out the columns in the analysis, so they can be scaled
                                                  cols.sc<- match(c("DoseEqPDE5","DoseEqERA","AgeAtDiag", "TAPSE","mPAP","FVC","PVR","BNP"), colnames(data.sc))
                                      ...
                                      # Define priors
                                                  fit0<- survreg(Surv(FUTimeYears, Dead == 1) ~ 1, data=d, dist="loglogistic")
                                                  theta0<- -c(fit0$coefficients[1], log(fit0$scale))
                                                  n<- c(10,5,5)[model]
                                                  prior=list(beta0=rep(0,n),S0=diag(3,n),theta0=theta0,V0=diag(3,2),maxL=15,a0=5,b0=1)
                                                                    
                                      # Bayesian survival regression
                                                          res.bayes[[model]][[i]]<- survregbayes(as.formula(paste("Surv(tstart, tstop, Outcome == 1)", newbit[model])),
                                                                                        data = data.sc,
                                                                                        InitParamMCMC = TRUE,
                                                                                        prior = prior,
                                                                                        mcmc=mcmc, survmodel = "AFT", 
                                                                                        dist = "loglogistic", subject.num = IDn) 
                                    } # Imputations
            ...
            # Pool coefficients
            matrix.mean<- matrix.sd<- coef.df<- list()
            imputations<- 100
            
            for (model in 1:3)
                {
                    matrix.mean[[model]]<- t(sapply(1:imputations, function(x) {summary(res.bayes[[model]][[x]])[[4]][,1]}))
                    matrix.sd[[model]]<- t(sapply(1:imputations, function(x) {summary(res.bayes[[model]][[x]])[[4]][,3]}))
                    ncoefs.survival.coefficients<- ncol(matrix.mean[[model]])
                    coef.df[[model]]<- pooling.by.Rubins.rules(matrix.mean[[model]], matrix.sd[[model]], imputations, ncoefs.survival.coefficients)
                    rownames(coef.df[[model]])<- colnames(matrix.mean[[model]])
                }
            
            o<- order(rownames(coef.df[[model]]))
